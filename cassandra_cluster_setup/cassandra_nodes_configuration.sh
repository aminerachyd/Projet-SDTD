...
ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook playbooks/cassandra_nodes_setup.yml -u $TF_VAR_ssh_username -i $(echo "$NODE1_PUB,") --private-key ~/.ssh/id_rsa -e "@NODE1_JSON.json"

ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook playbooks/cassandra_nodes_setup.yml -u $TF_VAR_ssh_username -i $(echo "$NODE2_PUB,") --private-key ~/.ssh/id_rsa -e "@NODE2_JSON.json"
